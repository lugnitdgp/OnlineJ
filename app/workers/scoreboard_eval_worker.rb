class ScoreboardEvalWorker
  ...
  def perform
    contests = Contest.where(start_time: { :$lte => DateTime.now } , end_time: { :$gt => DateTime.now-10.minutes })
    if contests.nil?
      return
    end
    contests.each do |contest|
      ...
      if total_submissions_count == ranklist.submissions_count
        next
      end
      ...
      contest_users.each do |user|
        ...
        contest_problems.each do |problem|
          ...
          if problem_submissions_ac.count > 0
            ...
            user_not_ac_count = problem_submissions.where(:status_code.nin => ["AC", "PE", "CE"], :submission_time.lte => user_ac_earliest_time).count
            ...
            user_wa_count = problem_submissions.where(:status_code.nin => ["AC", "PE", "CE"]).count
